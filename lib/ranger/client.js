...
exports.Client = function (account, token) {
  ...
  this.room = function (id, callback) {
    connection.get(urlFor("room", id), function (err, data) {
      if (err) return callback(err)
      callback(null, new Room(connection, data.room));
    });
  };
  ...
  this.rooms = function (callback) {
    connection.get(urlFor("rooms"), function (err, data) {
      if (err) return callback(err)
      callback(null, toRooms(data));
    });
  };
  ...
  this.presence = function (callback) {
    connection.get(urlFor("presence"), function (err, data) {
      if (err) return callback(err)
      callback(null, toRooms(data));
    });
  };
  ...
  this.search = function (term, callback) {
    connection.get(urlFor("search", term), function (err, data) {
      if (err) return callback(err)
      var i, messages = [];
  ...
  this.user = function (id, callback) {
    connection.get(urlFor("users", id), function (err, data) {
      if (err) return callback(err)
      callback(null, new User(connection, data.user));
    });
  };
  ...
  this.me = function (callback) {
    connection.get(urlFor("users", "me"), function (err, data) {
      if (err) return callback(err)
      callback(null, new User(connection, data.user));
    });
  };
