  ...
  this.stream = function (path, connectionCallback, callback) {
    ...
    var request = https.request(options, function (response) {
      if (response.statusCode >= 400) {
        reconnect = null;
        return callback(new Error("Got error code: " + response.statusCode))
      }
      ...
      var reconnect = function(why) {
        reconnect = null;
        console.log("reconnecting to room:", path, why)
        self.stream(path, connectionCallback, callback)
      }
      ...
      response.on("end", function () {
        ...
        if (callback) {
          try {
            ...
            return callback(new Error("Unable to parse response: " + data))
          }
